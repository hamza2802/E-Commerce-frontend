...
@Component({
  selector: 'app-category-headphones',
  templateUrl: './category-headphones.component.html',
  styleUrls: ['./category-headphones.component.css']
})
export class CategoryHeadphonesComponent implements OnInit {
  products: any[] = [];
  headphones: any[] = [];
  ...
  ngOnInit(): void {
    this.productService.getAllProducts().subscribe(
      (data) => {
        ...
        this.extractBrands();
        this.maxPrice = Math.max(...this.headphones.map((p) => p.productDiscountedPrice));
        this.priceRange.max = this.maxPrice;
        this.filteredHeadphones = [...this.headphones];
      },
      (error) => {
  ...
  extractBrands(): void {
    this.brands = Array.from(
      new Set(this.headphones.map((product) => product.productName.split(' ')[0]))
    );
    this.brands.sort();
  }

  applyFilters(): void {
    let filtered = this.headphones.filter((product) => {
      const matchesBrand =
